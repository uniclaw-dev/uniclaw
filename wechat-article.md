...
面对已经成熟的 Claude Code，一个自然的疑问是：与其从零构建新的 Agent 系统，不如直接赋予它通讯的能力？UniClaw 正是这个尝试——用500行代码，为现有的终端智能体装上"虾爪"。
...
UniClaw 的架构设计充分利用了 Chrome DevTools Protocol 和本地进程间通信：

![UniClaw 架构图](https://uniclaw.dev/architecture.svg)

### 数据流详解

一条消息从发送到回复的完整生命周期：

![UniClaw 数据流](https://uniclaw.dev/datapath.svg)

**流程说明**：
1. **消息发送**：用户在聊天网页版（Chrome 9222）发送 `jarvis: status`
2. **消息同步**：聊天服务器同步消息，Chrome 9223（无头浏览器）同步收到
3. **消息检测**：`uniclaw-agent.js` 每10秒查询 9223，发现新消息
4. **任务分发**：解析前缀 `jarvis:`，确定目标为 Pane 1
5. **本地投递**：`notify-other-session.sh` 使用 AppleScript 选择 iTerm Pane 1，写入任务并回车
6. **任务执行**：Claude Code 在 Pane 1 接收并执行
7. **结果回写**：结果写入 `~/uniclaw/task_result_jarvis`
8. **结果采集**：Agent 轮询发现结果，读取内容
9. **响应发送**：通过 Chrome 9223 的 CDP 在聊天网页版输入并发送结果
10. **消息可见**：用户在 Chrome 9222 中看到回复
